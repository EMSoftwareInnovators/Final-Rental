...
const info = await page.evaluate(() => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    c.z = 6.30; c.x = 6.62; c.yaw = Math.PI;
    c.anim.legSwing = 0.30; c.anim.armL = -0.26; c.anim.armR = 0.26; c.anim.bob = 0;
    window.__parade.push(c);
    out.push({ name: c.name, tag: c.personality.tag, h: c.app.height.id, build: c.app.build.id,
...
for (let i = 0; i < 5; i++) {
  ...
  await page.screenshot({ path: `shots/char-${i + 1}.png`, clip: { x: 330, y: 170, width: 364, height: 500 } });
}
console.log(info.map((c, i) => `${i + 1}. ${c.name} (${c.tag})\n   ${c.h}/${c.build} | ${c.hair} | hat:${c.hat} glasses:${c.glasses} mark:${c.mark}\n   ${c.jacket} + ${c.pants} | carrying:${c.carry}`).join('\n'));
await browser.close();
